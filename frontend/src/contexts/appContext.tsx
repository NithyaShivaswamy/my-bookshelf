...
const ContextProvider = ({ children }: PropsWithChildren) => {
  ...
  const localTheme = localStorage.getItem("books-theme");
  ...
  const [books, setBooks] = useState<IBook[]>(
    JSON.parse(sessionStorage.getItem("books-list") || "[]")
  );

  const [searchedBooks, setSearchedBooks] = useState<IBook[]>(
    JSON.parse(sessionStorage.getItem("books-searched") || "[]")
  );
  ...
  useEffect(() => {
    if (query) {
      sessionStorage.setItem("books-query", query);
    }

    if (!query && submittedSearch) {
      setSearchParams("");
      setFilteredBooks(books);
      sessionStorage.removeItem("books-query");
    }
  ...
  useEffect(() => {
    const sessionQuery = sessionStorage.getItem("books-query");
  ...
  useEffect(() => {
    ...
    sessionStorage.setItem(
      "books-favorites-array",
      JSON.stringify(favoritesList)
    );
    sessionStorage.setItem("books-favorites", JSON.stringify(favorites));
  }, [favoritesList, books]);

  const fetchBooks = useCallback(() => {
    const sessionBooks = JSON.parse(
      sessionStorage.getItem("books-list") || "[]"
    );

    const favoritesArray = JSON.parse(
      sessionStorage.getItem("books-favorites-array") || "[]"
    );

    const favorites = JSON.parse(
      sessionStorage.getItem("books-favorites") || "[]"
    );

    const searched = JSON.parse(
      sessionStorage.getItem("books-searched") || "[]"
    );
    ...
    if (!sessionBooks.length) {
      ...
      AxiosInstance.get("/api/books")
        .then((res) => {
          ...
          sessionStorage.setItem("books-list", JSON.stringify(res.data));
        })
        .catch((err) => {
          setAreBooksLoading(false);
